...
if (typeof L === "undefined") {
  console.error(
    "Leaflet library (L) not found. Make sure it is loaded via CDN or import."
  );
} else {
...
export function initMap(
  mapId,
  centerCoords = [-6.99117, 110.43458],
  zoomLevel = 7
) {
  if (!L) return null;
  ...
  if (mapElement._leaflet_id) {
    console.warn(
      `Map container "${mapId}" already has a Leaflet instance (_leaflet_id: ${mapElement._leaflet_id}). Attempting removal.`
    );
    if (mapInstance && typeof mapInstance.remove === "function") {
      try {
        ...
        mapInstance.remove();
        console.log(
          `Removed previous map instance referenced by mapInstance variable.`
        );
      } catch (e) {
        console.error("Error removing previous mapInstance:", e);
      }
    } else {
      mapElement.innerHTML = "";
      console.warn(
        `Map container "${mapId}" cleared directly as mapInstance variable was unavailable.`
      );
    }
    mapInstance = null;
    layerControl = null;
  ...
  try {
    ...
    const streetsTileLayer = L.tileLayer(CONFIG.MAP_TILE_LAYER_URL, {
      maxZoom: 19,
      attribution:
        '&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a> contributors &amp; <a href="https://www.maptiler.com/">MapTiler</a>',
    ...
    mapElement.innerHTML =
      '<p class="error-message" style="padding: 10px;">Failed to Load the Map</p>';
    mapInstance = null;
    return null;
  }
}
...
export function addMarker(coords, popupContent = null, options = {}) {
  ...
  try {
    ...
    if (error.message.includes("appendChild")) {
      console.error(
        "Leaflet Error Hint: This often relates to missing marker icon images. Ensure L.Icon.Default.imagePath is set correctly."
      );
    }
    return null;
  }
...
export function clearMarkers() {
  ...
  activeMarkers.forEach((marker) => {
    if (mapInstance.hasLayer(marker)) {
      try {
        ...
        console.warn("Could not remove active marker:", e);
      }
    }
  });
  activeMarkers = [];
  ...
  if (singleInputMarker && mapInstance.hasLayer(singleInputMarker)) {
    try {
      mapInstance.removeLayer(singleInputMarker);
    } catch (e) {
      console.warn("Could not remove singleInputMarker:", e);
    }
  }
  singleInputMarker = null;
}
...
export function setupMapInput(
  mapId,
  initialCoords = [-6.99117, 110.43458],
  ...
  map.on("click", (e) => {
    ...
    singleInputMarker.off("dragend").on("dragend", function (event) {
      ...
      marker
        .setPopupContent(
          `Selected Location : ${newCoords.lat.toFixed(
            5
          )}, ${newCoords.lon.toFixed(5)}`
        )
        .openPopup();
  ...
  const existingInstruction = map
    .getContainer()
    .querySelector(".map-instruction-control");
  if (!existingInstruction) {
    const info = L.control({ position: "bottomleft" });
    info.onAdd = function (mapRef) {
      this._div = L.DomUtil.create(
        "div",
        "map-instruction map-instruction-control"
      );
      this.update();
      return this._div;
    };
